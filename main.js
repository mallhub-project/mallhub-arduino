...
const serial = async (
        ...
        poolBancoDados = mysql.createPool(
            {
                ...
                password: "#Gfgrupo5",
                database: "bd-mallhub",
                server: "srv-mallhub.database.windows.net",
                pool: {
                    max: 10,
                    min: 0,
    ...
    arduino.pipe(new serialport.ReadlineParser({ delimiter: '\r\n' })).on('data', async (data) => {
        ...
        const luminosidade = parseFloat(valores[4]);
        const lm35Temperatura = parseFloat(valores[3]);
        const chave = parseInt(valores[2]);

        valoresDht11Umidade.push(1);
        valoresDht11Temperatura.push(1);
        valoresLuminosidade.push(1);
        valoresLm35Temperatura.push(1);
        valoresChave.push(chave);

        if (HABILITAR_OPERACAO_INSERIR) {
            if (AMBIENTE == 'producao') {
                if (chave == 0 || chave == 1) {
                    console.log(chave, 'É IGUAL A NUMERO')
                    sqlquery = `INSERT INTO evento (nome, chave, fk_dispositivo) VALUES ('teste DO NAN', ${chave}, 1)`;

                    // CREDENCIAIS DO BANCO REMOTO - SQL SERVER
                    const connStr = "Server=srv-mallhub.database.windows.net;Database=bd-mallhub;User Id=usuarioParaAPIArduino_datawriter;Password=#Gf_grupo5;";
                ...
                console.log("valores inseridos no banco: teste", dht11Umidade ? dht11Umidade : 0 + ", " + dht11Temperatura + ", " + luminosidade + ", " + lm35Temperatura + ", " + chave)
